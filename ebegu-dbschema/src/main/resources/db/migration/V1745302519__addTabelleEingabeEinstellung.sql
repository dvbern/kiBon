...
INSERT INTO einstellung (id, timestamp_erstellt, timestamp_mutiert, user_erstellt, user_mutiert, version,
	...
	(
		SELECT UNHEX(REPLACE(UUID(), '-', '')) AS id,
			...
			'0' AS version,
			'TABELLE_EINGABEMASKE' AS einstellungkey,
			'false' AS value,
			id AS gesuchsperiode_id,
			'Bei einem Erstantrag erscheint eine Tabelle analog der Abweichungsmaske in der die Betreuungspensen pro Monat erfasst werden können.' AS erklaerung
		FROM gesuchsperiode
	);
