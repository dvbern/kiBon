...
INSERT INTO einstellung (id, timestamp_erstellt, timestamp_mutiert, user_erstellt, user_mutiert, version,
	...
	(
		SELECT UNHEX(REPLACE(UUID(), '-', '')) AS id,
			...
			'0' AS version,
			'SOZIALABZUG_PRO_KIND' AS einstellungkey,
			'6700' AS value,
			id AS gesuchsperiode_id,
			'Wenn bei der Einstellung "Kinderabzugstyp" der Inputwert "Kanton Schwyz" eingestellt ist, so kann mit dieser Einstellung bestimmt werden, wie hoch (in CHF) der Kinderabzug pro Kind ist' AS erklaerung
		FROM gesuchsperiode
	);
