...
INSERT INTO einstellung (id, timestamp_erstellt, timestamp_mutiert, user_erstellt, user_mutiert, version,
	...
	(
		SELECT UNHEX(REPLACE(UUID(), '-', '')) AS id,
			   ...
			   '0' AS version,
			   'BETREUUNG_COMPARATOR' AS einstellungkey,
			   'DEFAULT' AS value,
			id AS gesuchsperiode_id,
			'In kiBon muss bei Doppelbetreuungen eine Priorität festgelegt werden. Die verschiedenen Prioritätenlisten sind im Confluence beschrieben: https://intra.dvbern.ch/spaces/KIB/pages/242943474/Doppelbetreuungen' AS erklaerung
		FROM gesuchsperiode
	);
