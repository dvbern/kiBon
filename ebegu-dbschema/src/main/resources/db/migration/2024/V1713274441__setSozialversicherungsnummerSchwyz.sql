...
INSERT INTO einstellung (id, timestamp_erstellt, timestamp_mutiert, user_erstellt, user_mutiert, version,
	...
	(
		SELECT UNHEX(REPLACE(UUID(), '-', '')) AS id,
			   ...
			   '0' AS version,
			   'SOZIALVERSICHERUNGSNUMMER_PERIODE' AS einstellungkey,
			   'false' AS value,
			   id AS gesuchsperiode_id,
		      'Angabe der Sozialversicherungsnummer je Antragsteller/in' AS erklaerung
		FROM gesuchsperiode
	);

update einstellung
set value = 'true'
WHERE einstellung_key = 'SOZIALVERSICHERUNGSNUMMER_PERIODE'
