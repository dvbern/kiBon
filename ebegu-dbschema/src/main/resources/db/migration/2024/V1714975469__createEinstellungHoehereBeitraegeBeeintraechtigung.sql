...
INSERT INTO einstellung (id, timestamp_erstellt, timestamp_mutiert, user_erstellt, user_mutiert, version,
	...
	(SELECT UNHEX(REPLACE(UUID(), '-', '')) AS id,
			...
			'0' AS version,
			'HOEHERE_BEITRAEGE_BEEINTRAECHTIGUNG_AKTIVIERT' AS einstellungkey,
			'false' AS value,
			id AS gesuchsperiode_id,
			'Aktiviert die Frage und Berechung "Höhere Beiträge für Kind mit Beeinträchtigung"' AS erklaerung
	 FROM gesuchsperiode);
...
SET value='true'
WHERE gesuchsperiode.mandant_id = @mandant_id_schwyz AND einstellung_key = 'HOEHERE_BEITRAEGE_BEEINTRAECHTIGUNG_AKTIVIERT';
