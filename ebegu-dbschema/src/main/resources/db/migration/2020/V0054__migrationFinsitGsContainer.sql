...
where famsit.id = famsitcontainer.familiensituationgs_id and famsitcontainer.id = gs.familiensituation_container_id and (gs.gesuchsteller1_id = gsc.id or gs.gesuchsteller2_id = gsc.id)
and gsc.id = fsc.gesuchsteller_container_id and fsc.finanzielle_situationgs_id = fs.id) = false;
