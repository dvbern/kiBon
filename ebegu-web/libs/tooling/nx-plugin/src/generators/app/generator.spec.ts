...
describe('app generator', () => {
    ...
    describe('app', () => {
        it('should generate new app and add new scope to module boundaries and generator', async () => {
            ...
            expect(
                (config.targets as any).build.options.assets[0].input
            ).toEqual('libs/shared/assets/i18n/src');
            expect(
                (config.targets as any).build.options.assets[1].input
            ).toEqual('libs/shared/assets/images/src');
            expect(
            ...
            expect(
                depConstraints.find(
                    (c: any) => c.sourceTag === 'scope:example-app'
                )
            ).toEqual({
