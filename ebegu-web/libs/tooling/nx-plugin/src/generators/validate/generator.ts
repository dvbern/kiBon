...
export default async function validate(
    ...
    for (const project of projects) {
        const {violations, fixes} =
            await validateProjectTagsMatchProjectLocation(tree, project, fix);
        aggregateFixes.push(...fixes);
        aggregateViolations.push(...violations);
    }

    for (const project of projects) {
        const validateSelectorsViolations = await validateSelectors(
            tree,
            project
        );
    ...
    return () => {
        if (aggregateViolations.filter(Boolean)?.length > 0) {
            ...
            console.error(
                chalk.red.bold(aggregateViolations.join('\n\n'), '\n\n')
            );
    ...
    findFiles(
        ...
        selectors.forEach(selector => {
            ...
            const normalizedNamespace = `${scope}${type}${name}`
                ...
                .toLowerCase();
            if (
                !normalizedSelector?.includes(normalizedNamespace) &&
                selector
            ) {
...
async function validateTsconfigBaseJson(
    ...
    const projectNamesWithoutTsconfigBaseJsonPath = libProjectNames.filter(
        projectName =>
            !tsconfigBaseJsonPathsAsProjectNames.includes(projectName)
    );
...
async function getModuleBoundaries(tree: Tree) {
    ...
    if (!boundaries) {
        throw new Error(
            `The definition for eslint rule "'@nx/nx/enforce-module-boundaries'" not found in the root .eslintrc.json, it should be the first item in the "overrides" array`
        );
    }
    return boundaries;
}
...
function getFoldersFromTree(tree: Tree, path: string) {
    ...
    const folders = tree
        ...
        .filter(
            path =>
                !IGNORE.some(ignore => path.includes(ignore)) &&
                !tree.isFile(path)
        );
    return folders;
}
