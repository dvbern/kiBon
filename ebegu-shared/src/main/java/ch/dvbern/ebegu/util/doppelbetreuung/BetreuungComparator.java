 ...
 */
public class BetreuungComparator implements
	...
	public int compare(AbstractPlatz platz1, AbstractPlatz platz2) {
		...
		List<BetreuungspensumContainer> betreuungenSorted1 = new LinkedList<>(
			betreuung1.getBetreuungspensumContainers()
		);
		List<BetreuungspensumContainer> betreuungenSorted2 = new LinkedList<>(
			betreuung2.getBetreuungspensumContainers()
		);

		Collections.sort(
			betreuungenSorted1,
			new BetreuungspensumContainerComparator()
		);
		Collections.sort(
			betreuungenSorted2,
			new BetreuungspensumContainerComparator()
		...
		Betreuungspensum firstBetreuungspensum1 = betreuungenSorted1.get(0)
			.getBetreuungspensumJA();
		Betreuungspensum firstBetreuungspensum2 = betreuungenSorted2.get(0)
			.getBetreuungspensumJA();

		// Regel 0: Untenstehendes gilt nur fuer JA-Angebote Kleinkind!
		if (betreuung1.getBetreuungsangebotTyp()
			.isAngebotJugendamtKleinkind()) {
			// 1. ist JA-Kleinkind, aber 2. nicht
			if (!betreuung2.getBetreuungsangebotTyp()
				.isAngebotJugendamtKleinkind()) {
				return -1;
			}
		} else if (betreuung2.getBetreuungsangebotTyp()
			.isAngebotJugendamtKleinkind()) {
		...
		int result = firstBetreuungspensum1.getGueltigkeit()
			.getGueltigAb()
			.compareTo(
				firstBetreuungspensum2.getGueltigkeit().getGueltigAb()
			);
		if (result == 0) {
			// Regel 2: Höheres Pensum
			result = firstBetreuungspensum2.getPensum()
				.compareTo(firstBetreuungspensum1.getPensum()); // Absteigend
			if (result == 0) {
				// Regel 3: Reihenfolge der Erfassung
				result = betreuung1.getBetreuungNummer()
					.compareTo(betreuung2.getBetreuungNummer());
			}
		}
		return result;
	}
