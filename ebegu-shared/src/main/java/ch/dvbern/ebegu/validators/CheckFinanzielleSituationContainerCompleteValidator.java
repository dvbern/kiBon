...
public class CheckFinanzielleSituationContainerCompleteValidator implements
	...
	private static final Logger LOG = LoggerFactory.getLogger(
		CheckFinanzielleSituationContainerCompleteValidator.class
			.getSimpleName()
	);
	...
	public boolean isValid(
		...
		if (finSitContainer.getFinanzielleSituationJA() == null) {
			LOG.error(
				"FinanzielleSituationJA is empty for FinanzielleSituationContainer {}",
				finSitContainer.getId()
			);
			valid = false;
		}
		if (finSitContainer.getGesuchsteller() == null) {
			LOG.error(
				"GesuchstellerContainer is empty for FinanzielleSituationContainer {}",
				finSitContainer.getId()
			);
			valid = false;
		}
